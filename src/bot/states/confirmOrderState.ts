...
export async function handleConfirmOrderState(client: Client, message: Message, session: UserSession): Promise<UserSession> {
  const confirm = message.body.trim().toLowerCase();
  const phone = message.from;
  if (confirm === "sí" || confirm === "si") {
    const item = session.cart![0];
    await prisma.pedidos.create({
      data: { cliente: { connect: { telefono: phone } }, cantidad: 1, observaciones: item.name },
    });
