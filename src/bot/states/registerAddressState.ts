...
export async function handleRegisterAddressState(client: Client, message: Message, session: UserSession): Promise<UserSession> {
  const address = message.body.trim();
  const phone = message.from;
  await prisma.clientes.upsert({
    where: { telefono: phone },
    update: { nombre: session.name!, direccion: address },
    create: { nombre: session.name!, direccion: address, telefono: phone, empresa_id: 1 },
  });
